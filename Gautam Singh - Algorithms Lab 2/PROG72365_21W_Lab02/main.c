...
int main(int argc, char* argv[])
{
	...
	for (i = 1; i < argc; ++i) {
		...
		bPackedString = PackToBits(argv[i], strlen(argv[i]), &iPackedLen);   // argv[i] is the ith char string input by user (i.e. 1100 ), i elements are separated by spaces
																			 // &iPackedLen is the memory location of the int iPackedLen which is passed to PackToBits()
																			 // and stores the number of Bytes requires to store the converted ASCII characters to bits.
																			 // E.g. '1' '0' '1' would be converted to 1010 0000 and iPackedLen would be 1.
																			 // E.g. '1' '0' '1' '0' '1' '1' '0' '1' '1' would be converted to 1010 1101 1000 0000 and iPackedLen would be 2.
